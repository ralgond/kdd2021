...
if __name__ == "__main__":
    ...
    os.system(f"md mp_selfjoin_output\{win_size}")
    for fn in os.listdir("samples"):
        ...
        max_profile, max_index, profile = mp_detect_selfjoin(test, win_size)

        top2_idx = topk(profile, 2, win_size)

        output_path = os.path.join(f"mp_selfjoin_output/{win_size}", f"{file_no}.txt")
